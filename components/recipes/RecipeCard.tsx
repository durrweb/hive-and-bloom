...
export default function RecipeCard({ recipe }: { recipe: RecipeWithMeta }) {
  ...
  return (
    <Link href={`/recipes/${recipe.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
      <article
        style={{ background: 'var(--cream)', borderRadius: 12, overflow: 'hidden', border: '1px solid var(--cream-dark)', transition: 'transform 0.22s, box-shadow 0.22s', cursor: 'pointer' }}
      ...
      >
        {/* Image area */}
        <div style={{ height: 160, background: recipe.cover_image_url ? `url(${recipe.cover_image_url}) center/cover` : `linear-gradient(135deg, ${honeyColor}33, ${honeyColor}11)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '4rem' }}>
        ...
        <div style={{ padding: '1.2rem 1.3rem 1.4rem' }}>
          ...
          {recipe.description && (
            <p style={{ fontSize: '0.9rem', color: 'var(--mist)', fontWeight: 300, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
              {recipe.description}
            </p>
          )}
