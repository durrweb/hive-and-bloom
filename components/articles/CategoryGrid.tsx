    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
      {categories.slice(0, 6).map(cat => {
        ...
        return (
          ...
          >
            <div style={{
              background: bg, borderRadius: 12, padding: '1.5rem 1.4rem',
            ...
            >
              ...
              <h3 style={{ fontFamily: 'var(--font-playfair)', fontSize: '1.25rem', fontWeight: 700, color: text, marginBottom: '0.4rem', letterSpacing: '-0.01em' }}>
                {cat.name}
              </h3>
              <p style={{ fontSize: '0.92rem', lineHeight: 1.55, fontWeight: 300, color: text, opacity: 0.75, marginBottom: '0.9rem' }}>
                {cat.description}
              </p>
              <span style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.8rem', fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: link, display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
                Browse guides →
              </span>
            </div>
          </Link>
